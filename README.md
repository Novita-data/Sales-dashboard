# Sales Performance Dashboard – Power BI

## Project Overview
This project presents an interactive Sales Performance Dashboard created using Power BI. The dashboard analyzes business sales data across regions, product categories, customer segments, and time periods to identify sales trends and business performance patterns.

The objective of this project is to transform raw sales data into meaningful business insights through data visualization and KPI analysis.

---

## Tools Used
- Power BI
- Power Query
- DAX (Data Analysis Expressions)
- Superstore Sales Dataset

---

## Dataset Information
The dataset contains sales transaction details including:
- Order ID
- Order Date
- Customer Segment
- Region
- Product Category
- Sub-Category
- Sales

The dataset was cleaned and transformed using Power Query before analysis.

---

## Key KPIs
The dashboard includes the following key performance indicators:

- Total Business Revenue
- Total Orders
- Total Customers

Custom DAX measures were created for KPI calculations.

---

## Business Questions Answered
This dashboard helps answer important business questions such as:

- Which region generates the highest sales?
- Which product category performs best?
- How do sales change over time?
- Which customer segment contributes most to revenue?
- Which sub-categories underperform in sales?

---

## Dashboard Features
### Sales Analysis by Region
Compares sales contribution across different business regions.

### Product Category Analysis
Evaluates category-wise sales performance.

### Time Trend Analysis
Tracks sales growth across quarters and time periods.

### Customer Segment Analysis
Identifies high-performing customer segments.

### Sub-Category Breakdown
Highlights strong and weak performing product sub-categories.

### Interactive Filtering
Region slicers allow dynamic dashboard interaction.

---

## Key Insights
- Technology category contributes significantly to overall sales performance.
- Consumer segment shows the highest sales contribution among customer groups.
- Regional analysis indicates stronger performance in East and West regions.
- Certain sub-categories contribute comparatively lower sales, indicating opportunities for improvement.

---

## Learning Outcomes
Through this project, I practiced:
- Data cleaning using Power Query
- Creating DAX measures
- Building KPI cards
- Designing interactive dashboards
- Business-oriented data storytelling
- Analytical thinking using sales data

---

## Dashboard Preview
(Add dashboard screenshot here)

---

## Author
[Your Name]
